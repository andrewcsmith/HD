...
module MM
  ...
  @@get_olm_search = ->(opts) {
    ...
    tuneable_data      = opts[:tuneable_data]     || get_tuneable_data(NArray.to_na(start_vector), angler, hd_config)
    ...
    # Generate a table of all possible costs (see 'get_tuneable_data.rb' for
    # more info on the cost function)
    ...
    max_iterations.times do |iter|
      begin
        ...
        begin
          ...
          possible_interval = HD.change_inner_interval(current_point, ind_y, HD.r(*best_interval))
          
          # Check to see whether the prospective point has beeen either banned
          # or is already in the current path (no infinite loop plz)
          # banned_points is a hash, since #has_key? has a quicker lookup than Array#include?
          while (banned_points.has_key? HD.narray_to_string possible_interval) || (path.include? possible_interval)
            ...
            if interval_index >= cost.size
              ...
              initial_run = true
              current_cost = NMath.sqrt(((get_coords.(path[-1], start_vector) - goal_vector) ** 2).sum)
              break
            end
          ...
          # If the interval we just discovered is banned, move to the next
          # index number. Otherwise, add it to the path.
        ...
        # pick the best first soon)
          ...
          previous_cost = NMath.sqrt(((get_coords.(path[-2], start_vector) - goal_vector) ** 2).sum)
          if (prospective_cost - current_cost) < (prospective_cost - previous_cost)
            current_cost = prospective_cost
          else
            # if not, add the point to banned_points
            banned_points[HD.narray_to_string path[-1]] = path[-1]
            path.pop
            initial_run = false
        ...
        if current_cost < epsilon
          ...
          tuneable_data[true,1,true,true] += get_coords.(current_point, start_vector)
          current_point = path[-1]
...
begin
  ...
  hd_config.tuneable.reject! {|x| x.to_f > 6.0}
  hd_config.reject_untuneable_intervals!
  ...
  (1..14).to_a.each do |x|
    ...
    f.puts "\nRESULTS for #{x}:\n#{results[-1][0].to_a}"
    f.puts "Lowest OLD: #{results[-1][2][:lowest_old]}"
