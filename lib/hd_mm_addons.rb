...
module MM
  ...
  # Not particularly efficient. This was taking about 4500 iterations to find
  # 10 candidates that were tuneable, and there are only 2500 possible
  # candidates. Might be more efficient to go through the whole list of
  # candidates and then select only some of them to run the dist_function on.
  @@get_stochastic_hd_search = ->(opts) {
    ...
    # Holds an array of all intervals that are the "wrong way" to go. There is
    # the assumption that if a point leads to a dead-end at one time, it will
    # always lead to a dead-end and that point should always be rejected.
    if banned
      wrong_way = banned
    else
      wrong_way = []
    end
    # dead_ends is a catalog of every dead end that has been found by the
    # search function. The keys are all points, while the values are tuneable
    # intervals. If a tuneable interval returns no possible tuneable points
    # (not equal to the current point) than it is deemed a dead end, and added
    # to that point's list of dead ends in the value array.
    dead_ends = {}
    climb_scores = {}
    ...
    max_iterations.times do |iteration|
      ...
      # Sorts the list of tuneable intervals by whether they can possibly
      # bring the vector closer The current point will be multiplied by the
      # tuneable interval, across the vector
      ...
      # Initialize a dead ends hash entry for the current point if it does not
      # already have one
      ...
      test_points = []
      # Sort the list of tuneable intervals by the mean hd-sum of all possible
      # tuneable combinations operating on the current vector
      scores = []
      ...
      # Raises a random tuneable interval to a random exponent from [-1,0,1]
      # and multiplies this by a dimension of the point. The process is
      # repeated for each dimension of the point. The advantage to this is
      # that it gives more musically interesting results, especially when
      # there are fewer iterations. (it uses a wider range of tuneable
      # intervals to find a point) The disadvantage is that it's incredible
      # slow, because of all the randomness and the low probability that a
      # randomly altered point will lead to a tuneable vector.
  ...
  @@get_hd_search = ->(opts) {
    ...
    max_iterations.times do |iteration|
      ...
      current_point = winner
      # The path is made up of two entries: 
      # [point, distance via path to the current point]
      path << current_point
